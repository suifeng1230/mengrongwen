...
if (PHP_VERSION < "5.2.0") {
	exit("PHP version need to be more than 5.2!");
}
...
if (!is_writable(__CORE_DIR . "config.php")) {
...
class Index extends Kernel
{
	...
	public function install()
	{
		...
		if (!file_exists(__APP_DIR . "data/jh_data.sql")) {
			exit("安装脚本缺失");
		}
		...
		if ($_POST["initdemo"] && file_exists(__APP_DIR . "data/jh_demo.sql")) {
		...
		foreach (explode(";\n", trim($sql)) as $query ) {
			...
			foreach ($queries as $query ) {
				$ret[$num] .= ((!isset($query[0]) && ($query[0] == "#")) || (($query[0] . $query[1]) == "--") ? "" : $query);
			}
		...
		$siteArr = array("title" => "博虎装修网门户系统", "mail" => "ijianghu@qq.com", "kfqq" => "1027527487", "phone" => "183-3520-0263", "siteurl" => "http://" . $host, "secret" => trim(base64_encode($secretkey), "="), "mobile" => "0", "ucenter" => "0", "city_id" => "7", "multi_city" => "0", "city_domain" => "", "rewrite" => "0", "intro" => "博虎装修网经历了一年半的发展从原先不了解这个行业到做出最适合这个行业营销的平台，江 湖是一直实事求是的和站长站在统一战线！2014年是垂直行业互联网创业的最佳年份 ，那么大同装修网无疑是家居垂直领域最适合站长的O2O系统！", "tongji" => "", "icp" => "皖ICP备13010842号");
