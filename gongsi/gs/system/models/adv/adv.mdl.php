        ...
        if($items = $detail['items']){
            ...
            if($adv['from'] == 'script'){
				...
				foreach($item_list as $k => $item){
					$content .= str_replace(array('<{$item.a_attr}>','<{$pager.img}>','<{$item.clickurl}>','<{$item.link}>','<{$item.title}>','<{$item.desc}>','<{$item.thumb}>','<{$item.item_attr}>'), array('target='."'".$item['target']."'",$cfg['url'],$item['link'],$item['link'],$item['title'],$item['desc'], $item['thumb'], $item['item_attr']), $match[1]);
				}
				if($match2){
