...
class Ctl_Mobile_Passport extends Ctl_Mobile
{
     ...
     * 手机注册
     */
    public function mobileAdd($from)
    {
        if($from=="member")
        {
            $check_phone = K::M('member/member')->check_phone($this->checksubmit('data')['phone']);
            if($check_phone)
            {
                ...
                if($data = $this->checksubmit('data')){
                    $res  = K::M('member/member')->mobileAdd($data);
                    header('Location:'.K::M('helper/link')->mklink('mobile/ucenter-index:'.$from));
                  return  $this->err->add('恭喜您，注册会员成功');
                }
            }
    ...
    public function byphone($from)
    {
        if($from == 'member'){
            if($data = $this->checksubmit('data')){
                ...
                if($code = $session->get('code_'.$data['phone'])){
                    if($data['code'] == $code){
                        if($items = K::M('member/member')->items(array('uname'=>$data['phone']),array('uid'=>'desc'),1,1)){
                            ...
                            // http://www.meng.com/index.php?ucenter/member-index-149
                        }else{
                ...
                $this->tmpl = 'mobile/passport/byphone.html';
            }
        }else{
...
//				$this->err->add('信息发送成功');
//			}
            require_once  IN_DIR.'system/plugins/payments/dysms/api_demo/SmsDemo.php';
            $datasms =  SmsDemo::sendSms($phone,$code);
            if($datasms->Message=='OK')
            {
                $this->err->add('信息发送成功');
            }
//        }
...
//        if($_SESSION['code']==$data['code']&&$_SESSION['phone']=$data['phone'])
//        {
//            if($uid = K::M('member/member')->mobileAdd($data)){
//
...
//        print_r($this->checksubmit('data'));die;
//    	if(!$this->checksubmit('data')){
//    		$this->err->add('非法的数据提交', 211);
//    	}else if(!$data = $this->GP('data')){
//            $this->err->add('非法的数据提交', 212);
