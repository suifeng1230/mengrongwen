...
class Ctl_Ucenter_Member_Verify extends Ctl_Ucenter
{
    ...
    public function mobile($loc=null)
    {
        //判断 签到
        if(is_numeric($loc)){  
            $info = K::M('member/member')->fatch($loc);
            $info_time = K::M('member/member')->fatch_time($loc,$info['atime']);
            if($info_time['is_time'] == 1 ){ 
                echo    '<script language="javascript" type="text/javascript"> 
                                alert("已签到");
                            window.location.href="/index.php?ucenter/member-index.html"; 
                        </script> ';
                exit;
            }else{
                if($info_time == null ){
                    $data['is_time'] = '1';
                    $data['atime'] = time();                   
                    $data['activity'] = $info['activity'] + 2;
                    

                    $rs = K::M('member/member')->update($loc,$data,true);                   
                    if($rs){                        
                         echo   '<script language="javascript" type="text/javascript"> 
                                        alert("签到成功");
                                    window.location.href="/index.php?ucenter/member-index.html"; 
