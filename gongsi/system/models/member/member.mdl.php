...
{    
    ...
    {
        $sql = "SELECT * FROM ".$this->table($this->_table)." WHERE mobile='$data' ";
        return $info = $this->db->GetRow($sql);
    }
    ...
    public function mobileAdd($data,$checked=false)
    {
        $uid = $this->db->Execute("INSERT INTO ".$this->table('member')."(mobile) VALUES('".$data['phone']."')");
        return ($uid);
    ...
    public function fatch_time($uid,$atime){
        ...
        $sql = "SELECT * FROM ".$this->table($this->_table)." WHERE uid='$uid' and $atime >'$start_stime' and $atime < $end_stime";
        return $this->db->GetRow($sql);
    }   
