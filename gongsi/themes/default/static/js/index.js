...
//首页装修公司排行
$(".inde_paih p.tat a").mouseover(function(){
    var index=$(this).index();
         $(".inde_paih p.tat a").each(function(a){ 
            if(a == index){
                $(this).addClass('current');
                $('.inde_paih .inde_paihang').find('ul').eq(a).show();
            }
            else{
                 $(this).removeClass('current');
                 $('.inde_paih .inde_paihang').find('ul').eq(a).hide();
             }
         })                
     });                   
$(".inde_paih p.tat a").eq(0).mouseover(); 
